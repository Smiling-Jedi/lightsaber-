"""
策略6开盘版：无阈值 + 下周一开盘卖出/下周一开盘买入
与策略6（收盘版）对比，验证开盘执行vs收盘执行的差异
"""
...
print("="*80)
print("策略6开盘版：无阈值 + 周一开盘卖出/周一开盘买入")
print("="*80)
...
def run_strategy_monday_open():
    ...
    for friday in fridays:
        ...
        target, should_trade = select_no_threshold(factors, holding)
    ...
    print("与策略6收盘版对比")
    print("="*60)
    print(f"收盘版净值：13.20x | 开盘版：{final_nav:.2f}x | 差距：{final_nav-13.20:+.2f}x")

    return {
        'strategy': '策略6开盘版：无阈值+周一开盘买卖',
        'final_nav': final_nav,
        'annual': annual,
        'max_dd': max_dd,
        'sharpe': sharpe,
...
if __name__ == '__main__':
    ...
    if result:
        ...
        df.to_csv('策略6_开盘版_结果.csv', index=False, encoding='utf-8-sig')
        print(f"\n✓ 结果已保存: 策略6_开盘版_结果.csv")
