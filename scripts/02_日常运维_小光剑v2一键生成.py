...
用法:
    cd lightsaber && source venv/bin/activate && python scripts/02_日常运维_小光剑v2一键生成.py
    ...
    for symbol in symbols:
        try:
            ...
            if resp.status_code != 200:
                ...
                continue
            html = resp.text.replace('href="/mini/v2/"', 'href="../index.html"')
...
def step_git_push():
    ...
    commit_msg = f"chore: v2 自动更新 {timestamp}"
...
def main():
    parser = argparse.ArgumentParser(
        description="小光剑 v2 一键生成",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  python scripts/02_日常运维_小光剑v2一键生成.py        # 快速: 刷新价格 + 静态化 + push
  python scripts/02_日常运维_小光剑v2一键生成.py --analyze  # 完整: + 形态分析
  python scripts/02_日常运维_小光剑v2一键生成.py --no-push   # 本地: 不推送到 GitHub
        """,
    )
    parser.add_argument(
        "--analyze", "-a",
    ...
    try:
        ...
        # Step 3: 静态化导出
        step += 1
        print_step(step, total_steps, "静态化导出 v2")
