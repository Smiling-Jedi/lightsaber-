"""
静态化导出 — 把小光剑 /mini/ 子系统冻结为 HTML 静态站点

用法:
    cd lightsaber && python scripts/99_静态化_导出mini.py

输出:
    docs/mini/
      ├── index.html              # 首页(原 /mini/)
      ├── stock/                  # 30 个详情页
      │   ├── HK_00700.html
      │   ├── ...
...
链接重写规则:
- index.html 里: /mini/stock/HK:00700  → stock/HK_00700.html
- stock/X.html 里: /mini/              → ../index.html
"""
import os
import re
import sys
...
# 输出目录
OUT_DIR = Path(project_root) / "docs" / "mini"
...
def rewrite_home_links(html: str) -> str:
    """首页里的详情页链接 /mini/stock/HK:00700 → stock/HK_00700.html"""
    # 正则匹配 /mini/stock/{MARKET}:{CODE}
    def replace(m):
...
def rewrite_detail_links(html: str) -> str:
    """详情页里的返回链接 /mini/ → ../index.html"""
    # 处理 href="/mini/" 和 href='/mini/'
    html = html.replace('href="/mini/"', 'href="../index.html"')
    html = html.replace("href='/mini/'", "href='../index.html'")
    return html
...
def main():
    ...
    for symbol in symbols:
        try:
            resp = client.get(f"/mini/stock/{symbol}")
            if resp.status_code != 200:
    ...
    print(f"然后浏览器打开 http://localhost:8000/index.html")
