...
2. **双底严格确认条件**：必须在两个相近低点后反弹，且反弹幅度超过5%，**且突破颈线并站稳3个交易日，且突破时成交量放大**，才能标记为"已确认"；如果当前价还在颈线下方，只能标记为"构筑中"或"突破待确认"
3. **头肩底严格确认条件**：必须在三个低点形成（左低-更低-右低），**且突破颈线并站稳3个交易日，且突破时成交量放大**，才能标记为"已确认"；如果当前价还在颈线下方，只能标记为"构筑中"或"突破待确认"
4. **上升通道严格确认条件**：必须至少3个以上的高低点连线形成清晰通道，且当前价在通道内运行，才能标记为"已确认"；只有2个点的连线只能标记为"疑似"
5. 如果价格无明显低点结构，只是在区间内震荡，识别为"矩形整理"或"无明显形态"——**"无明显形态"是优先选择，不要硬套形态**
6. 不要同时给出多个可能的形态，只选择最可能的一种
7. 不要给出具体仓位百分比（如"加仓30%"），只给方向性建议（如"可分批加仓"/"继续持有"/"观望"）
8. 不要臆测数据范围外的结论；如不确定某项请用"数据不足以判断"
9. 关键价位要精确到具体数字
10. 不要主观估算 PE/PB 等估值数据（K线数据不含估值信息）
11. 【禁止事后诸葛亮】如果形态突破颈线已经超过2个月，当前价远离颈线>20%，不得标记"已确认"，应标记"已完成"或删除该形态
12. 【好公司滤镜提醒】如果标的属于知名优质公司（腾讯、阿里、比亚迪、宁德等），形态识别需更保守——突破后需额外技术指标佐证（如MACD/EMA方向一致）才能确认
13. 【非常重要】summary 必须包含具体价位数字，禁止只写"上轨压力线"、"区间低点"这种模糊表述。
